...
// Extrae los headers de cuota/rate-limit de una respuesta y los deja
// en un formato legible, para no tener que buscarlos en DevTools.
function extraerInfoCuota(headers) {
    const get = (nombre) => (typeof headers.get === "function" ? headers.get(nombre) : headers[nombre]);

    const quotaReset = get("x-quota-reset");
    const rateLimitReset = get("x-ratelimit-reset");
    const retryAfter = get("retry-after");

    return {
        quotaLimit: get("x-quota-limit"),
        quotaRemaining: get("x-quota-remaining"),
        quotaResetEn: quotaReset ? new Date(Number(quotaReset) * 1000).toISOString() : null,
        rateLimitLimit: get("x-ratelimit-limit"),
        rateLimitRemaining: get("x-ratelimit-remaining"),
        rateLimitResetEn: rateLimitReset ? new Date(Number(rateLimitReset) * 1000).toISOString() : null,
        retryAfterSegundos: retryAfter || null
    };
}
...
async function llamarAPI(url) {
    ...
    const infoCuota = extraerInfoCuota(respuesta.headers);

    if (!respuesta.ok) {
        console.warn(`${respuesta.status} ${url}`);
        console.warn("Info de cuota:", infoCuota);
    }
    ...
    // Si la API devolvió un error, adjuntamos la info de cuota al cuerpo
    // para que el frontend la vea sin tener que inspeccionar headers.
    if (!respuesta.ok && typeof body === "object" && body !== null) {
        body.infoCuota = infoCuota;
    }
...
app.post("/api/lwatlas/map-scan/jobs", async (req, res) => {
    try {
        ...
        if (!response.ok && typeof body === "object" && body !== null) {
            const infoCuota = extraerInfoCuota(response.headers);
            body.infoCuota = infoCuota;
            console.warn("Job de Map Scan falló. Info de cuota:", infoCuota);
        }
